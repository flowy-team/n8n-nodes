...
export async function updateKeyResult(
	...
	// Required fields
	body.krs_owner = this.getNodeParameter('krs_owner', index) as string;
	body.krs_title = this.getNodeParameter('krs_title', index) as string;
	body.krs_init = this.getNodeParameter('krs_init', index) as string;
	body.krs_now = this.getNodeParameter('krs_now', index) as string;
	body.krs_tar = this.getNodeParameter('krs_tar', index) as string;

	// Additional fields
	const additionalFields = this.getNodeParameter('additionalFields', index, {}) as IDataObject;
	Object.assign(body, additionalFields);

	const responseData = await ApiRequest.call(this, 'PUT', `/key-result/${keyResultId}`, body);
